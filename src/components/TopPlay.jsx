...
const artistImages = [artist1, artist2, artist3, artist4, artist5];
...
const TopPlay = () => {
  ...
  <SwiperSlide
    key={artist?.data?.[0]?.id}
    style={{ width: '25%', height: 'auto' }}
    className="shadow-lg rounded-full animate-slideright"
  >
    <Link to={`/artists/${artist?.data?.[0]?.id}`} key={artist?.data?.[0]?.id}>
      <img 
        src={
          artist?.images?.background 
            ? artist.images.background 
            : artistImages[i % artistImages.length]  // now i is defined
        } 
